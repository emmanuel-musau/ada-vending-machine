...
export default function VendingMachine() {
  ...
  const loadScriptUtxos = useCallback(async () => {
    ...
    setSelectedUtxo((prev) => (prev === null && data.length > 0 ? data[0] : prev))
  }, [])
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      >
        ...
        <pre className="text-green-500 text-xs mb-4 select-none">
{`╔════════════════════════════╗
║   ADA VENDING MACHINE v1   ║
...
╚════════════════════════════╝`}
        ...
        <div className="mb-4">
          <div className="text-green-600 text-xs mb-2 uppercase tracking-widest">
            [ CURRENT STOCK ]
          </div>
          {scriptUtxos.length === 0 ? (
            <div className="text-gray-600 text-sm">machine is empty</div>
          ) : (
        ...
        {/* Separator */}
        <div className="text-green-900 text-xs mb-4">
          ──────────────────────────────
        </div>

        {/* State display */}
        <div
          className={`text-center text-2xl font-bold mb-4 tracking-widest ${
            isDispensed
              ? "text-green-400"
              : isRejected
              ? "text-red-500"
        ...
        {(vendState === "idle" || vendState === "error") && (
          <div className="space-y-3">
            <input
              className="w-full bg-black border border-green-700 text-green-300 text-center text-xl px-4 py-3 focus:outline-none focus:border-green-400 tracking-widest"
              type="number"
              value={guess}
            ...
            <button
              ...
              disabled={!connected || !guess || isPending || scriptUtxos.length === 0}
              className="w-full py-3 border border-green-500 text-green-400 hover:bg-green-950 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-lg tracking-widest"
            >
              ▶ INSERT
            </button>

            {errorMsg && (
              <div className="text-red-400 text-xs px-3 py-2 border border-red-900 bg-red-950/30">
                {errorMsg}
              </div>
            )}
          </div>
        ...
        {isDispensed && txHash && (
          <div className="space-y-3 text-center">
            ...
            <button
              onClick={reset}
              className="border border-green-700 text-green-600 px-4 py-2 text-xs hover:bg-green-950"
            >
              TRY AGAIN
            </button>
          </div>
        ...
        {isRejected && (
          <div className="space-y-3 text-center">
            <div className="text-red-400 text-sm px-3 py-2 border border-red-900 bg-red-950/30">
              {errorMsg ?? "Wrong code — the validator rejected your guess."}
            </div>
            <button
              onClick={reset}
              className="border border-red-800 text-red-600 px-4 py-2 text-xs hover:bg-red-950"
            >
              TRY AGAIN
            </button>
          </div>
